    ...
    sliders = [
        dict(
            ...
            steps=[
                dict(
                    method='animate',
                    args=[[f"{round(fa,2):.2f}_{round(ff0,2):.2f}_{yr0}"],
        ...
        dict(
            ...
            steps=[
                dict(
                    method='animate',
                    args=[[f"{round(fa0,2):.2f}_{round(ff,2):.2f}_{yr0}"],
                          dict(mode='immediate', frame=dict(duration=0, redraw=True))],
                    label=f'{int(ff*100)}%'
                ) for ff in GRID_F
            ],
            currentvalue=dict(prefix='Factor crecimiento flota (f): ')
        ),
        dict(
            active=2,
            steps=[
                dict(
                    method='animate',
                    args=[[f"{round(fa0,2):.2f}_{round(ff0,2):.2f}_{int(yr)}"],
                          dict(mode='immediate', frame=dict(duration=0, redraw=True))],
                    label=str(int(yr))
                ) for yr in GRID_YR
            ],
            currentvalue=dict(prefix='Año intervención: ')
        )
    ]
